...
```console
$ docker run -v /myvalkey/conf:/usr/local/etc/valkey --name myvalkey public.ecr.aws/valkey/valkey valkey-server /usr/local/etc/public.ecr.aws/valkey/valkey.conf
```
...
View [license information](https://github.com/valkey-io/valkey/blob/unstable/COPYING) for the software contained in this image. test
