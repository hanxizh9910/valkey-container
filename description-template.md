...
```console
$ docker run --name some-valkey -d {container_repo_path}
```

## Start with persistent storage

```console
$ docker run --name some-valkey -d {container_repo_path} valkey-server --save 60 1 --loglevel warning
```
...
```console
$ docker run -it --network some-network --rm {container_repo_path} valkey-cli -h some-valkey
```
...
```console
$ docker run --env VALKEY_EXTRA_FLAGS='--save 60 1 --loglevel warning' {container_repo_path}
```
...
```console
$ docker run -v /myvalkey/conf:/usr/local/etc/valkey --name myvalkey {container_repo_path} valkey-server /usr/local/etc/valkey/valkey.conf
```
...
## `{container_repo_path}:<version>-alpine`
